...
def mif_weight_converter(path):
    ...
    if pth_data.type().find('FloatTensor')>0:
        ...
        if pth_data.shape[1] == 3:
            print('First Layer Change')
            # print('pth_data shape :', pth_data.shape)
            new_data = torch.zeros(pth_data.shape[0], 48)
            pth_data = pth_data.permute(0,3,1,2).reshape(pth_data.shape[0], -1)
            # print('pth_data shape :', pth_data.shape)
            new_data[:, :pth_data.shape[1]] = pth_data
            ...
            if pth_data.shape[3] == 3:
                ...
                new_data[:, :pth_data.shape[1]] = pth_data
                ic_unroll = new_data.shape[1] // 16
                new_data = new_data.reshape(pth_data.shape[0], ic_unroll, 16, 3, 3)
                new_data = new_data.permute(0, 1, 4, 2, 3).reshape(-1)
                new_data = np.uint8(new_data)
                ...
                new_data = new_data.reshape(new_data.shape[0], -1, 48)
                new_data = np.uint8(new_data)
