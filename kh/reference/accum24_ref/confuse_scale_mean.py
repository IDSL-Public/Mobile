...
def mif_converter(path, path2):
    pth_data = torch.load(path).cpu()
    pth_data2 = torch.load(path2).cpu()
    pth_data = (pth_data + pth_data2)/2
    ...
    conf_mid = path.find('mean/')

    torch.save(pth_data, './origin_data/confusion' + path[conf_mid + 4:])
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(".pth"):
                ...
                front_num = pth_path.find('pth/')
                pth_path2 = pth_path[:front_num] + pth_path[front_num + 4:]
                mif_converter(pth_path, pth_path2)
