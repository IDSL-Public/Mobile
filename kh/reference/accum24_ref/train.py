...
class NetLoss(nn.Module):
    ...
    def forward(self, images, targets, masks, num_crowds):
        preds = self.net.forward(images.cuda())
        losses = self.criterion(self.net, preds, targets, masks, num_crowds)
        return losses
        
    def backward(self, lr, weight_decay, momentum):
        self.criterion.zero_grad()
        #self.net.semantic_seg_conv.zero_grad()
        # self.net.bbox_layer.zero_grad()
        # self.net.conf_layer.zero_grad()
        # self.net.mask_layer.zero_grad()
        self.net.backward(lr, weight_decay, momentum)
...
def train():
    ...
    idx0_priors = torch.tensor(pd.read_csv('./0_priors.csv').values)
    idx1_priors = torch.tensor(pd.read_csv('./1_priors.csv').values)
    idx2_priors = torch.tensor(pd.read_csv('./2_priors.csv').values)
    idx3_priors = torch.tensor(pd.read_csv('./3_priors.csv').values)
    idx4_priors = torch.tensor(pd.read_csv('./4_priors.csv').values)
    priors = [idx0_priors, idx1_priors, idx2_priors, idx3_priors, idx4_priors]
    yolact_net = Yolact(priors)
    net = yolact_net
    net.train()
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                #print('\n\ndatum\n', datum[0][0],'\n\n')
                input_images = torch.stack(datum[0], dim=0)
                
                if cnt < iteration:
                    cnt += 1
                    print('cnt', cnt, '\titeration', iteration)
                    print('input_images :', input_images.shape)
                else:
                    cnt += 1
                    losses = net(input_images, datum[1][0], datum[1][1], datum[1][2])   
                    ...
                    # Backprop
                    loss.backward(retain_graph=True) # Do this to free up vram even if loss is not finite
                    #net.backward(args.lr, args.decay, args.momentum)
                    net.backward(args.lr, args.decay, args.momentum)
