...
def intersect(box_a, box_b):
    ...
    A = box_a.size(1)
    B = box_b.size(1)
    max_xy = torch.min(box_a[:, :, 2:].unsqueeze(2).expand(n, A, B, 2).cuda(),
                       box_b[:, :, 2:].unsqueeze(1).expand(n, A, B, 2).cuda())
    min_xy = torch.max(box_a[:, :, :2].unsqueeze(2).expand(n, A, B, 2).cuda(),
                       box_b[:, :, :2].unsqueeze(1).expand(n, A, B, 2).cuda())
...
def jaccard(box_a, box_b, iscrowd:bool=False):
    ...
    union = area_a.cuda() + area_b.cuda() - inter.cuda()
...
def match(pos_thresh, neg_thresh, truths, priors, labels, crowd_boxes, loc_t, conf_t, idx_t, idx, loc_data):
    ...
    """
    if cfg.use_prediction_matching:     #Deactivated
        decoded_priors = decode(loc_data, priors, cfg.use_yolo_regressors)  
    else: 
        decoded_priors = point_form(priors)     # pixel의 min-Max를 통해 box를 생성
    
    # Size [num_objects, num_priors]
    if not cfg.use_change_matching:
        overlaps = jaccard(truths, decoded_priors)          #jaccard를 통해 2 box 간 intersection을 계산
    else:                                                   #Deactivated
        overlaps = change(truths, decoded_priors)

    # Size [num_priors] best ground truth for each prior
    best_truth_overlap, best_truth_idx = overlaps.max(0)    # intersection이 가장 높은 값을 추출하여 best_truth_overlap, best_truth_idx로 뽑아냄
    ...
    if crowd_boxes is not None and cfg.crowd_iou_threshold < 1:
        ...
        conf[(conf <= 0) & (best_crowd_overlap.cpu() > cfg.crowd_iou_threshold)] = -1
...
def encode(matched, priors, use_yolo_regressors:bool=False):
    ...
    if use_yolo_regressors:
        ...
        center_size(matched)
        boxes = torch.load('/home/kh/reference/ref_resnet18_yolact/loss/box.pth')
        loc = torch.cat((
            boxes[:, :2] - priors[:, :2],
    ...
    '''
    #else:
    variances = torch.tensor([0.1, 0.2]).cuda()
    priors = priors.cuda()
