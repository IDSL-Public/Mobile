...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio):
        ...
        self.pos_threshold = pos_threshold          #Pos_threshold is given by cfg
        self.neg_threshold = neg_threshold          #neg_threshold is given by cfg
        self.negpos_ratio = negpos_ratio            #negpos is given by cfg
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        score_data = predictions['score'] if cfg.use_mask_scoring   else None       #Deactivated value
        inst_data  = predictions['inst']  if cfg.use_instance_coeff else None       #Deactivated value
        ...
        loc_t = loc_data.new(batch_size, num_priors, 4)             #Prediction
        gt_box_t = loc_data.new(batch_size, num_priors, 4)          #Ground-Truth
        conf_t = loc_data.new(batch_size, num_priors).long()        #Confidence score
        idx_t = loc_data.new(batch_size, num_priors).long()         #index?

        if cfg.use_class_existence_loss:            #Deactivated function
            class_existence_t = loc_data.new(batch_size, num_classes-1)


        for idx in range(batch_size):                               #idx means batch index
            truths      = targets[idx][:, :-1].data                 #.data means Escape from Autograd gradient
            labels[idx] = targets[idx][:, -1].data.long()           #targets is ground-truth

            if cfg.use_class_existence_loss:            #Deactivated
            ...
            cur_crowds = num_crowds[idx]                                # number of annotations
            if cur_crowds > 0:
                split = lambda x: (x[-cur_crowds:], x[:-cur_crowds])    #Truths, mask 마지막 부분에 crowds_boxes를 포함
                crowd_boxes, truths = split(truths)                     #Truth를 crowd_boxes와 truths로 분리

                # We don't use the crowd labels or masks
                _, labels[idx] = split(labels[idx])                     # Label의 crowd_box는 사용 x
                _, masks[idx]  = split(masks[idx])                      # Mask의 crowd_box는 사용 x
            else:
                crowd_boxes = None
        ...
        #Memory free
        #del conf_t, loc_t, idx_t
        #del pos, pos_idx, num_pos
        #del total_num_pos
        ...
        for idx in range(batch_size):
            ...
            with torch.no_grad():
                ...
                for obj_idx in range(downsampled_masks.size(0)):
                    segment_t[cur_class_t[obj_idx]] = torch.max(segment_t[cur_class_t[obj_idx]].cuda(), downsampled_masks[obj_idx].cuda())
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        #del conf_p, pos_idx, neg_idx, neg, num_pos
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, interpolation_mode='bilinear'):
        ...
        for idx in range(mask_data.size(0)):
            ...
            if cfg.mask_proto_mask_activation == activation_func.sigmoid:
                pre_loss = F.binary_cross_entropy(torch.clamp(pred_masks.cuda(), 0, 1), mask_t.cuda(), reduction='none')
            else:
